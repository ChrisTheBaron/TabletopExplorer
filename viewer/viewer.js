...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const topic = 'tabletop-explorer';//TODO Get this from query string
    ...
    console.log("Waiting 5 seconds before requesting first frame");
    setTimeout(() => {
        if (peers.length > 0) {
            gsub.publish(topic, new TextEncoder().encode('REQUEST'));
        } else {
            console.log("No peers.Waiting 5 seconds before requesting first frame");
            setTimeout(() => {
                if (peers.length > 0) {
                    gsub.publish(topic, new TextEncoder().encode('REQUEST'));
                }
